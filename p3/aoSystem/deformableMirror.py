...
class deformableMirror:
    ...
    def __init__(self, nActu1D, pitch, heights=[0.0], mechCoupling=[0.2],
                 modes='gaussian', opt_dir=[[0.0],[0.0]], opt_weights=[1.0],\
                 opt_cond=1e2, n_rec=None, validActuator=None, offset = [[0,0]],
                 AoArea='circle', resolution=None, precision='double'):
        ...
        # geometry
        self.nActu1D      = np.array(nActu1D, dtype=self.dtype)
        self.pitch        = np.array(pitch, dtype=self.dtype)
        self.heights      = np.array(heights, dtype=self.dtype)
        self.nDMs         = len(pitch)

        if (len(self.nActu1D) != self.nDMs) or (len(self.heights) != self.nDMs):
            raise ValueError("Please have the same size for nActu1D,"
                             "pitch and heights input lists.")

        # influence function
        self.mechCoupling = np.array(mechCoupling, dtype=self.dtype)
        if len(self.mechCoupling) < self.nDMs:
            self.mechCoupling = np.full(shape=self.nDMs, fill_value=self.mechCoupling[0], dtype=self.dtype)
            print("The first value of mechanical coupling ", \
                  self.mechCoupling[0] ," is set for all DMs")
        ...
        # optimization
        self.opt_dir      = np.array(opt_dir, dtype=self.dtype)
        self.opt_weights  = np.array(opt_weights, dtype=self.dtype)
        ...
        if np.any(validActuator == None):
            self.validActuator = [np.ones((nActu1D[kObj],nActu1D[kObj]), dtype=bool) \
    ...
    def __repr__(self):
        ...
        for kObj in range(self.nDMs):
            height_fmt = ".1f" if self.heights[kObj] == 0 else ".0f"
            s += (
                f" {kObj:d}\t {self.nActu1D[kObj]:d}\t\t {self.pitch[kObj]:.4f}\t\t "
                f"{float(self.heights[kObj]):{height_fmt}}\t\t {self.modes}\t {self.mechCoupling[kObj]:.2f}\t\t "
                f"{self.nValidActuator[kObj]:d}\n"
            )
        s +='----------------------------------------------------------------------------------------------\n'
